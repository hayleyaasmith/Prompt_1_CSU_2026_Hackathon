...
MANUAL_SOURCES = [
    {
        ...
        "instructions": dedent("""
            ...
            5. Each download is a ZIP containing one CSV. Unzip and rename to:
                  data/raw/t100_international_<YEAR>.csv
    ...
    },
    {
        ...
        "instructions": dedent("""
            ...
            6. Click "Run Query" → "Export to Excel" or "Export to CSV".
               Save as data/raw/gats_host_imports.csv
    ...
    },
    {
        ...
        "instructions": dedent("""
            ...
            3. Use the "Export" button (top right) → CSV.
               ...
               Transient). Save to:
                  data/raw/eppo_ceratitis_capitata.csv
                  data/raw/eppo_bactrocera_dorsalis.csv
                  data/raw/eppo_anastrepha_ludens.csv
    ...
    },
    {
        ...
        "expected_files": ["wc2.1_10m_tavg.zip", "wc2.1_10m_prec.zip"],
        "instructions": dedent("""
            ...
            3. Save the zips as-is to data/raw/. The cleaning step will unzip and clip
    ...
    },
    {
        ...
        "expected_files": ["cdl_2024_clipped.tif"],
        "instructions": dedent("""
            ...
            3. Download the most recent year (2024 as of writing). Save as:
                  data/raw/cdl_2024.tif
    ...
    },
    {
        ...
        "expected_files": ["aphis_validation_notes.md"],
        "instructions": dedent("""
...
def check_status() -> None:
    ...
    all_files = []
    for src in AUTO_SOURCES:
        all_files.append((src["filename"], "auto"))
    for src in MANUAL_SOURCES:
        for f in src["expected_files"]:
            all_files.append((f, "manual"))
    for fname, kind in all_files:
        path = RAW / fname
        if path.exists():
            size = path.stat().st_size
            print(f"  ✓ [{kind}]  {fname}  ({size:,} bytes)")
        else:
            print(f"  ✗ [{kind}]  {fname}  (missing)")
    print()
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="Report what's already in data/raw.")
